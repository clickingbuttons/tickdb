...
//
// V8 v10.3.174.6
fn fmt_error(scope: &mut v8::HandleScope, msg: v8::Local<v8::Message>, file: &str) -> String {
	let mut res = String::from(file);
	...
	if line.is_some() {
		let line = line.unwrap().to_rust_string_lossy(scope);
		res.push_str(&line);
	...
	if stack_trace.is_some() && stack_trace.unwrap().get_frame_count() > 0 {
		...
		for i in 0..stack_trace.get_frame_count() {
			...
			res.push_str("\tat ");
			let fname = match frame.get_function_name(scope) {
...
pub fn run_query<'a>(query: &Query) -> std::io::Result<Vec<u8>> {
	...
	let runtime_fn = v8::String::new(scope, RUNTIME_FN_NAME).unwrap();
	let runtime_fn = global.get(scope, runtime_fn.into()).unwrap();
