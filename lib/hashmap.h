...
static char* hm_next(hashmap* hm, char* cur) {
	char* it = cur + hm->key_size + hm->val_size;
	for (; it < hm_end(hm); it += hm->key_size + hm->val_size) {
...
#define hm_iter(hm) \
	for (char *it = hm_next(hm, hm_begin(hm)), *key = it, *val = it + (hm)->key_size; \
			 it < hm_end(hm); \
			 it = hm_next(hm, it), key = it, val = it + (hm)->key_size)
