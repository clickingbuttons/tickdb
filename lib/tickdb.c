...
void tickdb_schema_add(tickdb_schema *schema, char *name,
...
static void table_write_data(tickdb_table *table, void *data, size_t size) {
  ...
  col->size += size;
  table->cur_col_index =
      (table->cur_col_index + 1) % table->schema.columns.size;
}

static inline tickdb_block *get_block(tickdb_table *table, int64_t symbol,
                                      int64_t epoch_nanos) {
  ...
  for (size_t i = 0; i < blocks->size; i++) {
    ...
    if (epoch_nanos >= b->ts_min &&
        table->largest_col * b->n_rows < table->schema.block_size) {
      return b;
    }
  }
...
void tickdb_table_write(tickdb_table *table, char *symbol,
                        int64_t epoch_nanos) {
  tickdb_block *block =
      get_block(table, tickdb_table_stoi(table, symbol), epoch_nanos);
  if (strlen(table->cur_partition.name) == 0 ||
    ...
    size_t written =
        strftime(table->cur_partition.name, TICKDB_MAX_PARTITIONFMT_LEN,
                 string_data(&table->schema.partition_fmt), &time);
    if (written == 0) {
    ...
    printf("%s %lu %lu\n", table->cur_partition.name,
           table->cur_partition.ts_min, table->cur_partition.ts_max);
...
#define register_writer(ty, name)                                              \
  void tickdb_table_write_##name(tickdb_table *table, ty value) {              \
    table_write_data(table, &value, sizeof(ty));                               \
  }

register_writer(int8_t, i8) register_writer(int16_t, i16)
    register_writer(int32_t, i32);
register_writer(int64_t, i64);
register_writer(uint8_t, u8);
register_writer(uint16_t, u16);
register_writer(uint32_t, u32);
register_writer(uint64_t, u64);
register_writer(float, f32);
register_writer(double, f64);
